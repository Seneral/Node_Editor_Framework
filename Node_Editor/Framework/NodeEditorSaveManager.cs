...
namespace NodeEditorFramework
{
	...
	public static partial class NodeEditorSaveManager
	{
		...
		internal static NodeCanvasSceneSave FindSceneSave(string saveName)
		{
			...
			{
				sceneSave =
					sceneSaveHolder.GetComponents<NodeCanvasSceneSave>()
									.ToList()
									.Find(
										 (NodeCanvasSceneSave save) =>
												 save.saveName == saveName || (save.savedNodeCanvas != null && save.savedNodeCanvas.name == saveName));
				if (sceneSave != null)
		...
		internal static NodeCanvasSceneSave FindOrCreateSceneSave(string saveName)
		{
			...
			NodeCanvasSceneSave sceneSave =
				sceneSaveHolder.GetComponents<NodeCanvasSceneSave>()
								.ToList()
								.Find((NodeCanvasSceneSave save) => save.saveName == saveName || save.savedNodeCanvas.name == saveName);
			if (sceneSave == null)
		...
		public static void SaveSceneNodeCanvas(string saveName, ref NodeCanvas nodeCanvas, bool createWorkingCopy, bool safeOverwrite = true)
		{
			...
			if (nodeCanvas.GetType() == typeof(NodeCanvas))
				throw new UnityException("Cannot save NodeCanvas: The NodeCanvas has no explicit type: '" + nodeCanvas.GetType().ToString() +
										"'. Please convert it to a valid type!");
		...
		public static void SaveNodeCanvas(string path, NodeCanvas nodeCanvas, bool createWorkingCopy, bool safeOverwrite = true)
		{
			...
			if (nodeCanvas.GetType() == typeof(NodeCanvas))
				throw new UnityException("Cannot save NodeCanvas: The NodeCanvas has no explicit type: '" + nodeCanvas.GetType().ToString() +
										"'. Please convert it to a valid type!");
			if (nodeCanvas.livesInScene)
				Debug.LogWarning("Attempting to save scene canvas " + nodeCanvas.name + " to an asset, scene object references may be broken!" +
								(!createWorkingCopy ? " No workingCopy is going to be created, so your scene save is broken, too!" : ""));
#if UNITY_EDITOR
			...
			{
				Debug.LogError("Trying to create a duplicate save file for '" + nodeCanvas.name + "'! Forcing to create a working copy!");
			...
			// Saves each node and its additional scriptable objects.
			for (int i = 0; i < nodeCanvas.nodes.Count; i++)
			{
				...
				// Saves each knobs and their additional scriptable objects.
				for (int j = 0; j < node.nodeKnobs.Count; j++)
				{
					NodeKnob knob = node.nodeKnobs[j];
			...
			UnityEditor.AssetDatabase.ImportAsset(canvasSave.savePath);
...
// TODO: Node Editor: Need to implement ingame-saving (Resources, AsssetBundles, ... won't work)
		...
		public static NodeCanvas LoadNodeCanvas(string path, bool createWorkingCopy)
		{
			if (!File.Exists(path))
			{
				throw new UnityException("Could not load the NodeCanvas: File at '" + path + "' does not exist!");
			}
		...
		/// <summary>
		/// Saves the given sub-assets array in the specified main asset.
		/// </summary>
		/// <param name="subAssets">The sub-assets array to save in the main asset.</param>
		...
		public static void AddSubAsset(ScriptableObject subAsset, ScriptableObject mainAsset)
		{
			if (subAsset != null && mainAsset != null)
			{
				...
				{
					Debug.LogError(unityException.Message);
				}
		...
		/// <summary>
		/// Clones the specified <see cref="ScriptableObject"/>, preserving its name.
		/// </summary>
		/// <typeparam name="T">The <see cref="ScriptableObject"/>'s type.</typeparam>
		/// <param name="scriptableObject">The original <see cref="ScriptableObject"/> to clone.</param>
		/// <returns></returns>
		private static T Clone<T>(T scriptableObject) where T : ScriptableObject
		{
