...
public class Node_Editor : EditorWindow {
    ...
    public const string editorPath = "Assets/Plugins/Editor/Node_Editor/";
    ...
    private static void OnScriptsReloaded() {
        /*
         * If this script is changed and then reloaded while a node editor window is open, that window breaks.
         * This forces the window to be recreated, so it's possible to work on this script while testing, BUT 
         * deletes any unsaved canvas.
         * 
         * @TODO: if the canvas hasn't been saved yet, save it in a temp-file that fixes the deletion problem.
         */
        //CreateEditor();
    }

    public void checkInit() {
        if (!initiated || nodeCanvas == null || nodeBox == null) {
    ...
    private void Calculate() {
        ...
        for (int roundCnt = 0; !limitReached; roundCnt++) { // Runs until every node that can be calculated are calculated
            limitReached = true;
            for (int workCnt = 0; workCnt < workList.Count; workCnt++) {
                Node node = workList[workCnt];
                if (node.Calculate()) { // finished Calculating, continue with the children
                ...
                else if (!workList.Contains(node)) { // Calculate returned false state (due to missing inputs / whatever), add it to check later
                    workList.Add(node);
                }
            }
        }
    ...
    private void InputEvents() {
        ...
        if (e.type == EventType.Repaint) { // Draw background when repainting
        ...
        if (e.type == EventType.MouseDown) {
            ...
            if (clickedNode != null) { // A click on a node
                if (e.button == 1) { // Right click -> Node Context Click
                    ...
                    if (nodeOutput != null) { // Output Node -> New Connection drawn from this
                        connectOutput = nodeOutput;
                        e.Use();
                    }
                    else { // no output clicked, check input
                        NodeInput nodeInput = clickedNode.GetInputAtPos(mousePos);
                        if (nodeInput != null && nodeInput.connection != null) { // Input node -> Loose and edit Connection
            ...
            else if (!sideWindowRect.Contains(mousePos)) { // A click on the empty canvas
                if (e.button == 2 || e.button == 0) { // Left/Middle Click -> Start scrolling
                    scrollWindow = true;
                    e.delta = new Vector2(0, 0);
                }
            ...
            if (connectOutput != null) { // Apply a connection if theres a clicked input
                if (clickedNode != null && !clickedNode.Outputs.Contains(connectOutput)) {	// If an input was clicked, it'll will now be connected
                    NodeInput clickedInput = clickedNode.GetInputAtPos(mousePos);
                    if (Node.CanApplyConnection(connectOutput, clickedInput)) { // If it can connect (type is equals, it does not cause recursion, ...)
            ...
            if (navigate) { // Draw a curve to the origin/active node for orientation purposes
                DrawNodeCurve(nodeCanvas.scrollOffset, (activeNode != null ? activeNode.rect.center : e.mousePosition));
                Repaint();
            }
            if (connectOutput != null) { // Draw the currently drawn connection
                DrawNodeCurve(connectOutput.GetKnob().center, e.mousePosition);
                Repaint();
            }
        }
        if (scrollWindow) { // Scroll everything with the current mouse delta
    ...
    public void ContextCallback(object obj) {
        switch (obj.ToString()) {
            case "calcNode":
                CalcNode.Create(new Rect(mousePos.x, mousePos.y, 200, 100));
                break;

            case "inputNode":
                InputNode.Create(new Rect(mousePos.x, mousePos.y, 100, 50));
                break;
    ...
    public Node NodeAtPosition(Vector2 pos) {
        ...
        for (int nodeCnt = nodeCanvas.nodes.Count - 1; nodeCnt >= 0; nodeCnt--) { // From top to bottom because of the render order (though overwritten by active Window, so be aware!)
    ...
    public void SaveNodeCanvas(string path) {
        ...
        for (int nodeCnt = 0; nodeCnt < nodeCanvas.nodes.Count; nodeCnt++) { // Add every node and every of it's inputs/outputs into the file. 
        ...
        string[] folders = path.Split(new[] { '/' }, StringSplitOptions.None);
    ...
    public void LoadNodeCanvas(string path) {
        ...
        for (int cnt = 0; cnt < objects.Length; cnt++) { // We only have to search for the Node Canvas itself in the mess, because it still hold references to all of it's nodes and their connections
